...
#define DATABASE_URL "FIREBASE_RTDB_URL" // Definir essa constante como a URL do RTDB do Firebase
...
// função que faz um update no RTDB (na pasta "tags") das distâncias calculadas e da sala na qual a tag está
...
// função que faz um get no RTDB (na pasta "tags") que retorna um booleano sobre o status do LED e do BUZZER (se devem ser ligados)
bool getActuatorsStatus(String _macAddress) {
  sendDataPrevMillis = 0;
  ...
  if (Firebase.ready() && signupOK && (millis() - sendDataPrevMillis > 15000 || sendDataPrevMillis == 0)) {
    ...
    // get do status dos atuadores
    if (Firebase.RTDB.getBool(&fbdo, (String("/tags/") + _macAddress + String("/actuators")))) {
      if (fbdo.dataType() == "bool") {
        bool actuatorsStatus = fbdo.boolData(); // booleano que está armazenda no endereço "/actuators"

        Serial.print("Actuators Status: ");
        Serial.println(actuatorsStatus);
...
void loop() {
  ...
  bool findTag = getActuatorsStatus(myMacAddress); // faz um get que retorna um booleano se os atuadores (LED e BUZZER) devem ser ligados
  
  while (findTag) { // enquanto o booleano for true,
    Serial.print("Find Tag: ");
    Serial.println(findTag);

    for (uint8_t i = 1; i < 8; i++) {
      // liga o LED
      bool r = ( (i  >> 2) & 1 ) * brightness;  
      bool g = ( (i  >> 1) & 1 ) * brightness;
      bool b = ( (i  >> 0) & 1 ) * brightness;
    ...
    tone(BUZZER, 440, 500); // toca o BUZZER
    delay(1000);
    findTag = getActuatorsStatus(myMacAddress); // atualiza se os atuadores devem continuar ligados (faz outro get no RTDB)
  }
